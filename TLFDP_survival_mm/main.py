...
from wsi_data import TCGAFolder, patient_kfold
...
class Experiment(object):
    ...
    def _build_dataset(self):
        print(self.infofile, self.featuredir)
        dataset = TCGAFolder(self.infofile, self.featuredir, self.nFeature, self.masks, self.model_mm)
        return dataset

    

    def cross_validation(self, n_folds=5, random_state=0, repeat = 1, 
                        logdir='exp'):
        ...
        for k in range(n_folds):
            ...
            train_set_labels = dataset.events[train_idx[k]]

            test_set = Subset(evalset, test_idx[k])
            test_projects = evalset.projects[test_idx[k]]
            test_projects = test_projects.astype(
            ...
            preds, times, events = fit(model,
                                ...
                                scheduler=scheduler,
                                logdir=logdir,
                                path=os.path.join(self.outdir, 'model_' + str(repeat) + '_' + str(k)), 
                                )
...
def main():
    ...
    parser.add_argument(
        ...
        default=5)
    parser.add_argument(
        "--repeats", help="number of repeats",
        default=5)
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default='./result')
    parser.add_argument(
        "--infofile", help="infofile input file",
    ...
    parser.add_argument(
        ...
        default=False)
    parser.add_argument(
        "--model_mm", help="use clilnical data, read from infofile",
        default=False)
    parser.add_argument(
        "--nClass", help="",
        default=2)
